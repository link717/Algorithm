...
const solution = (cacheSize, cities) => {
  ...
  if (!cacheSize) {
    ...
    let lowerCases = cities.map(city => city.toLowerCase());

    for (let city of lowerCases) {
      let isLeng = cacheLists.length < cacheSize;
      let idx = cacheLists.indexOf(city);

      if (isLeng && idx === -1) {
        totalTime += 5;
        cacheLists.push(city);

      } else if ((isLeng && idx !== -1) || (!isLeng && idx !== -1)) {
        cacheLists.splice(idx, 1);
        totalTime += 1;
        cacheLists.push(city);
